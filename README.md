...
- `rf_email_spam_model.pkl`, `*.pth`, `*.safetensors` — model binaries (tracked by Git LFS)

## Quick start
1. Create a Python venv and install dependencies (PyTorch, snntorch, transformers, scikit-learn, joblib).
2. Ensure model files (`*.pth`, `*.safetensors`, `rf_email_spam_model.pkl`) are pulled via Git LFS.
3. Run `main.ipynb` or import `CNN_module.cnn_engine` / `SNN_module.snn_engine` / `Transformer_module.transformer_engine`.

## Notes
- Large dataset files are intentionally ignored. Model binaries are stored using Git LFS.
